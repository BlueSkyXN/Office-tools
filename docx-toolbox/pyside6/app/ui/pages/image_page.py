"""图片分离页 — task_type: image_extract"""

from PySide6.QtWidgets import (
    QWidget, QVBoxLayout, QGroupBox, QFormLayout, QCheckBox, QLabel, QSpinBox,
)
...
class ImagePage(QWidget):
    ...
    def _setup_ui(self):
        ...
        self._cb_remove_images = QCheckBox("删除原图并仅保留标记 (remove_images)")
        self._cb_optimize_images = QCheckBox("启用图片优化 (optimize_images)")
        og_layout.addWidget(self._cb_remove_images)
        og_layout.addWidget(self._cb_optimize_images)
        ...
        form.addRow("JPEG 质量 (jpeg_quality):", self._spin_quality)
    ...
    def _gather_options(self) -> dict:
        return {
            "remove_images": self._cb_remove_images.isChecked(),
            "optimize_images": self._cb_optimize_images.isChecked(),
    ...
    def _on_start(self):
        ...
        self.status_message.emit("运行中: 图片分离…")
        self._worker.start()
    ...
    def _on_reset(self):
        ...
        self._cb_remove_images.setChecked(False)
        self._cb_optimize_images.setChecked(False)
        self._spin_quality.setValue(85)
